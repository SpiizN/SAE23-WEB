...
function login($login, $password)
{
    try {
        ...
        if (VarInTab($tab_login, $login)) {
            ...
            $tab_password = request_login(
                "SELECT password FROM login WHERE login=" . $login . ";"
            );
...
function afficheTableauAssocImages($tab)
{
    try {
        echo "<table>";
        echo "<tr>";
        ...
        foreach ($tab as $ligne) {
            echo "<tr>";
            foreach ($ligne as $valeur) {
                if (strpos($valeur, "media/images") === 0) {
                    echo "<td><img src='$valeur' alt='Image' style='width:100px;'></td>";
                } else {
                    echo "<td>$valeur</td>";
                }
            }
...
function ajouterProduit(
    ...
    $requete = $connexion->prepare(
        "INSERT INTO produit (designation, idCat, prixTTC, forfaitLivraison, images) VALUES (?, ?, ?, ?, ?)"
    );
...
function listerCategories()
{
    ...
    $resultat = $connexion->query(
        "SELECT idCat, intitule FROM categorieproduit"
    );
...
function listerForfaitsLivraison()
{
    ...
    $resultat = $connexion->query(
        "SELECT idForfait, description FROM forfaitlivraison"
    );
